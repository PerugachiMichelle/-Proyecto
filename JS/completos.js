class ListarCompletos extends HTMLElement {
    ...
    actualizarCursoCompletado = async (id_estudiante, id_curso) => {
        const nuevaFecha = prompt('Ingrese la nueva fecha de completado (YYYY-MM-DD):');

        // Validación simple de formato de fecha
        const regexFecha = /^\d{4}-\d{2}-\d{2}$/;
        if (!nuevaFecha || !regexFecha.test(nuevaFecha)) {
            alert('Fecha no válida. Asegúrese de que el formato sea YYYY-MM-DD.');
            return;
        }
        ...
        try {
            const response = await fetch(`http://localhost:8000/cursos-completados/${id_estudiante}/${id_curso}`, {
                ...
                body: JSON.stringify({ fecha_completado: nuevaFecha }),
            });

            if (response.ok) {
                alert('Curso completado actualizado con éxito');
                this.obtenerCompletos(this.getAttribute('api-url')); // Recargar la lista
            } else {
                alert('Error al actualizar el curso completado');
            }
        } catch (error) {
            console.error('Error al actualizar el curso completado:', error);
            alert('Hubo un problema al actualizar el curso completado. Intente más tarde.');
        }
    };
            ...
            html += `
                <tr>
                    ...
                    <td class="botones">
                        ...
                        <button class="boton actualizar" onclick="document.querySelector('listar-completos').actualizarCursoCompletado(${completo.id_estudiante}, ${completo.id_curso})">Agregar</button>
                    </td>
                </tr>
            `;
        });
